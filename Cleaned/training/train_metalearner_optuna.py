...
def objective(trial):
    ...
    params = {
        ...
        "EPOCHS": 15,
        "LOAD_MODEL": False,
        "LOSS_FUNCTION": trial.suggest_categorical("LOSS_FUNCTION", [dice_loss, cross_entropy_loss, iou_loss, focal_loss, CExDL025, CExDL05, CExDL075, DLxFL025, DLxFL05, DLxFL075]),
        "KERNEL_POOL_SIZE": 2,
        "CONV_KERNEL_SIZE": 2,
        "DEEP_SUPERVISION": False,
        "SCHEDULER": "step_lr",
        "UNET_FEATURES": trial.suggest_categorical("UNET_FEATURES", [[64, 128, 256, 512, 1024], [32, 64, 128, 256, 512]]),
    ...
    # Appel à la fonction train_unet
    train_unet(**params)
    # Chargement du dernier checkpoint pour évaluation
    checkpoint = torch.load(params["SAVENAME"] + ".pth")
    model.load_state_dict(checkpoint["state_dict"])
    model.eval()
    _, test_dataset = get_datasets(**params)
    test_loader = DataLoader(test_dataset, batch_size=params["BATCH_SIZE"], shuffle=False)
    results = evaluate(model, test_loader, params["DEVICE"])
    return results["eval_dice_score"].item()
...
if __name__ == "__main__":
    ...
    print("Meilleur Dice Score:", study.best_trial.value)
